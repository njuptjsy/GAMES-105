...
def calculateJacobian(end_position, joint_angle, path_positions, path_orientations):
    ...
    for i in range(len(joint_angle)):
        ...
        #base vector for local transfrom
    ...
    jacobian = np.concatenate(jacobian, axis=0).transpose()#转置
    return jacobian
...
def dampedGaussNewtonMethod(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    while (np.linalg.norm(path_positions[end_index] - target_pose) >= 1e-2 and count <= 100):
        ...
        # theta = theta_0 - alpha J^T (JJ^T + lambda W)^-1 delta
        jacobianSquare = np.dot(jacobian, jacobian.transpose())
        temp = jacobianSquare + lambda_ * np.ones(jacobianSquare.shape)
        temp = np.linalg.inv(temp)#(JJ^T + lambda W)^-1
        temp = np.dot(jacobian.transpose(), temp)
